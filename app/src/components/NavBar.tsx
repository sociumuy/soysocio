...
const items = [
  {
    ...
    href: '/home',
    icon: (active: boolean) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill={active ? '#B8975A' : 'none'}
        stroke={active ? '#B8975A' : '#666'} strokeWidth="1.6" strokeLinecap="round">
        ...
        {items.map((item) => {
          ...
          return (
            <motion.button
              ...
              style={{ background: active ? 'rgba(184,151,90,0.12)' : 'transparent' }}
            >
              {item.icon(active)}
              <span className="text-[9px] font-bold tracking-widest uppercase"
                style={{ color: active ? '#B8975A' : '#555' }}>
                {item.label}
              </span>
              {active && (
                <motion.div
                  layoutId="dock-indicator"
                  className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[#B8975A]"
